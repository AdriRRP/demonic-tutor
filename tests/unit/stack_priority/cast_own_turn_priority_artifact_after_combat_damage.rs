...
//! Unit coverage for unit stack priority cast own turn priority artifact after combat damage.
...
#[test]
fn active_player_can_cast_and_resolve_an_own_turn_priority_artifact_after_combat_damage() {
    let (service, mut game) = setup_two_player_game(
        "game-own-turn-artifact-after-combat-damage",
        filled_library(
            vec![
    ...
    let attacker_id = CardInstanceId::new("game-own-turn-artifact-after-combat-damage-player-1-0");
    service
        .cast_spell(
            &mut game,
    ...
    service
        .cast_spell(
            ...
            CastSpellCommand::new(
                PlayerId::new("player-1"),
                CardInstanceId::new("game-own-turn-artifact-after-combat-damage-player-1-1"),
            ),
        )
        .unwrap();
