...
#[test]
fn non_active_player_cannot_cast_an_own_turn_priority_artifact_as_a_response() {
    let (service, mut game) = setup_two_player_game(
        "game-respond-own-turn-artifact",
    ...
    service
        .cast_spell(
            ...
            CastSpellCommand::new(
                ...
                CardInstanceId::new("game-respond-own-turn-artifact-player-1-0"),
            ),
        )
        .unwrap();
    ...
    let result = service.cast_spell(
        ...
        CastSpellCommand::new(
            ...
            CardInstanceId::new("game-respond-own-turn-artifact-player-2-0"),
        ),
    );
    ...
    assert!(matches!(
        ...
        Err(DomainError::Game(GameError::CastingTimingNotAllowed { card, permission }))
            if card == CardInstanceId::new("game-respond-own-turn-artifact-player-2-0")
