...
//! Unit coverage for unit stack priority cast artifact in main window.

use {
    crate::support::{
        advance_to_first_main_satisfying_cleanup, advance_to_phase_satisfying_cleanup,
        artifact_card, filled_library, setup_two_player_game,
    },
    demonictutor::{
...
#[test]
fn active_player_can_cast_and_resolve_an_artifact_in_first_main() {
    let (service, mut game) = setup_two_player_game(
        "game-first-main-artifact",
    ...
    let outcome = service
        .cast_spell(
            ...
            CastSpellCommand::new(
                ...
                CardInstanceId::new("game-first-main-artifact-player-1-0"),
            ),
        )
        .unwrap();
...
#[test]
fn active_player_can_cast_and_resolve_an_artifact_in_second_main() {
    let (service, mut game) = setup_two_player_game(
        "game-second-main-artifact",
        filled_library(vec![artifact_card("ivory-cup", 0)], 10),
        filled_library(Vec::new(), 10),
    );
    ...
    let outcome = service
        .cast_spell(
            ...
            CastSpellCommand::new(
                PlayerId::new("player-1"),
                CardInstanceId::new("game-second-main-artifact-player-1-0"),
            ),
        )
        .unwrap();
