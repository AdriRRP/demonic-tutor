...
//! Unit coverage for unit stack priority cast flash creature after blockers.

use {
    crate::support::{
        advance_to_player_first_main_satisfying_cleanup, close_empty_priority_window,
        filled_library, flash_creature_card, resolve_top_stack_with_passes, setup_two_player_game,
    },
    demonictutor::{
...
#[test]
fn active_player_can_cast_a_flash_creature_after_blockers_are_declared() {
    let (service, mut game) = setup_two_player_game(
        "game-flash-after-blockers",
        filled_library(
            vec![
                LibraryCard::creature(CardDefinitionId::new("attacker"), 0, 2, 2),
                flash_creature_card("ambush-viper", 0, 2, 1),
            ],
            10,
        ),
        filled_library(
    ...
    let attacker_id = CardInstanceId::new("game-flash-after-blockers-player-1-0");
    service
        .cast_spell(
            &mut game,
    ...
    let blocker_id = CardInstanceId::new("game-flash-after-blockers-player-2-0");
    service
        .cast_spell(
            &mut game,
    ...
    service
        .cast_spell(
            ...
            CastSpellCommand::new(
                PlayerId::new("player-1"),
                CardInstanceId::new("game-flash-after-blockers-player-1-1"),
            ),
        )
        .unwrap();
    ...
    assert_eq!(
        ...
        "game-flash-after-blockers-player-1-1"
    );
}
