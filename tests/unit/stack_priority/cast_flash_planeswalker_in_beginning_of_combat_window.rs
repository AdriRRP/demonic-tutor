...
//! Unit coverage for unit stack priority cast flash planeswalker in beginning of combat window.
...
#[test]
fn active_player_can_cast_and_resolve_a_flash_planeswalker_at_beginning_of_combat() {
    let (service, mut game) = setup_two_player_game(
        "game-flash-planeswalker-boc",
    ...
    service
        .cast_spell(
            ...
            CastSpellCommand::new(
                PlayerId::new("player-1"),
                CardInstanceId::new("game-flash-planeswalker-boc-player-1-0"),
            ),
        )
        .unwrap();
