...
//! Unit coverage for unit stack priority nonlethal controlled blocking target damage.
...
#[test]
fn controlled_blocking_creature_spell_marks_nonlethal_damage_and_leaves_the_blocker_in_combat() {
    let (service, mut game) = setup_two_player_game(
        "game-target-controlled-blocking-nonlethal",
        filled_library(
        ...
        filled_library(
            vec![
                LibraryCard::creature(CardDefinitionId::new("blocker"), 0, 2, 3),
                targeted_controlled_blocking_creature_damage_instant_card("shield-snap", 0, 1),
            ],
            10,
        ),
    );
    ...
    let attacker_id = CardInstanceId::new("game-target-controlled-blocking-nonlethal-player-1-0");
    service
        .cast_spell(
            &mut game,
    ...
    let blocker_id = CardInstanceId::new("game-target-controlled-blocking-nonlethal-player-2-0");
    service
        .cast_spell(
            &mut game,
