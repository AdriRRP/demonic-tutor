...
//! Unit coverage for unit stack priority cast second instant in draw window.
...
#[test]
fn active_player_can_cast_a_second_instant_before_passing_in_draw() {
    let (service, mut game) = setup_two_player_game(
        "game-second-draw-instant",
        filled_library(
            vec![instant_card("draw-a", 0), instant_card("draw-b", 0)],
            10,
        ),
    ...
    service
        .cast_spell(
            ...
            CastSpellCommand::new(
                PlayerId::new("player-1"),
                CardInstanceId::new("game-second-draw-instant-player-1-0"),
            ),
        )
        .unwrap();
    service
        .cast_spell(
            ...
            CastSpellCommand::new(
                PlayerId::new("player-1"),
                CardInstanceId::new("game-second-draw-instant-player-1-1"),
            ),
        )
        .unwrap();
