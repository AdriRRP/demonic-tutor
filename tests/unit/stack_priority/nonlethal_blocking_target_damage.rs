...
//! Unit coverage for unit stack priority nonlethal blocking target damage.
...
#[test]
fn targeted_blocking_creature_spell_marks_nonlethal_damage_and_leaves_the_blocker_in_combat() {
    let (service, mut game) = setup_two_player_game(
        "game-target-blocking-nonlethal",
        filled_library(
            vec![
                LibraryCard::creature(CardDefinitionId::new("attacker"), 0, 2, 2),
                targeted_blocking_creature_damage_instant_card("hold-the-line", 0, 1),
            ],
            10,
        ),
        filled_library(
    ...
    let attacker_id = CardInstanceId::new("game-target-blocking-nonlethal-player-1-0");
    service
        .cast_spell(
            &mut game,
    ...
    let blocker_id = CardInstanceId::new("game-target-blocking-nonlethal-player-2-0");
    service
        .cast_spell(
            &mut game,
    ...
    service
        .cast_spell(
            ...
            CastSpellCommand::new(
                ...
                CardInstanceId::new("game-target-blocking-nonlethal-player-1-1"),
            )
