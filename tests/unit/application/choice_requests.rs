...
#[test]
fn choice_requests_surface_pending_scry_for_supported_scry_spell() {
    let service = create_service();
    let libraries = vec![
        player_library(
            "p1",
    ...
    let scry_id = player(&game, "p1")
        ...
        .expect("scry spell should be in hand")
        .id()
        .clone();

    service.execute_public_command(
        &mut game,
        PublicGameCommand::CastSpell(CastSpellCommand::new(PlayerId::new("p1"), scry_id.clone())),
    );
    service.execute_public_command(
        &mut game,
        PublicGameCommand::PassPriority(PassPriorityCommand::new(PlayerId::new("p1"))),
    ...
    assert!(requests.iter().any(|request| matches!(
        request,
        PublicChoiceRequest::PendingScry { player_id, source_card_id, looked_at_card_ids, options }
            if player_id.as_str() == "p1"
