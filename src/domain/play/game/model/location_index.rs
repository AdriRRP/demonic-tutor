...
impl AggregateCardLocation {
    #[must_use]
    pub const fn new(owner_index: usize, zone: PlayerCardZone) -> Self {
        Self { owner_index, zone }
    }
...
impl AggregateCardLocationIndex {
    ...
    pub fn refresh(&mut self, players: &[Player]) {
        ...
        for (owner_index, player) in players.iter().enumerate() {
            for (card_id, zone) in player.owned_card_locations() {
                self.by_card_id.insert(
                    card_id.clone(),
                    AggregateCardLocation::new(owner_index, zone),
                );
            }
        }
    }
