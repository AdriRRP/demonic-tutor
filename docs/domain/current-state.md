...
- the current non-combat targeted-spell subset explicitly exercises `opponent player`, `creature you control`, and `creature an opponent controls` restrictions in `FirstMain`, including cast-time rejection and successful resolution for the currently modeled opponent-controlled-creature damage corridor
