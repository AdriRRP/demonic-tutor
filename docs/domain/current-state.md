...
- minimal colored mana support for `Forest -> Green`, `Mountain -> Red`, `Plains -> White`, and single-color instant costs
...
- mana production is simplified to generic mana plus a minimal colored subset (`White`, `Green`, `Red`) and is currently exercised in main phases plus the currently supported open priority windows while the acting player holds priority
