...
The current host-loss implementation completes that first remote horizon by turning terminal authority loss into a read-only ended remote session with explicit recovery options instead of letting the peer keep pretending the match can continue.
...
- a viewer-scoped opening-hand overlay with seat-aware hero states so each device keeps its own hand visible while the active chooser gets clear keep, mulligan, and bottoming affordances, including numbered bottom seals directly on the selected hand cards
