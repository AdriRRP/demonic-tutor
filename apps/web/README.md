...
`npm run check` is the frontend quality gate used by CI: format, strict lint, focused component tests, production build, `npm audit`, and dependency freshness against the pinned manifest.
...
- a viewer-scoped pregame overlay with stronger seat-aware hero states so each device sees only its own opening hand while waiting, deciding, or bottoming after mulligans, with numbered seals directly on the selected bottom cards
...
- supported browsers can also import those signaling payloads back from QR images
